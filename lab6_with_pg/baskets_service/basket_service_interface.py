...
async def basket_create(token : str = Body(...)):
    ...
    basket_id = basket_coll.insert_one({"owner_user_id" : int(verify[1]), "time_opened" : str(datetime.now()), "basket_contents" : []}).inserted_id
...
async def basket_add_item(  token : str = Body(...),
    ...
    if int(doc["owner_user_id"]) != int(verify[1]):
